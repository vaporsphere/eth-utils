#!/bin/bash
# Usage:
# bash /path/to/vap-utils/gvapup.sh <datadir> <instance_name>
...
# TODO: use this if GVAP not set
# GVAP=gvap

# gvap CLI params       e.g., (dd=04, run=09)
datetag=`date "+%c%y%m%d-%H%M%S"|cut -d ' ' -f 5`
datadir=$root/data/$dd        # /tmp/vap/04
log=$root/log/$dd.$datetag.log     # /tmp/vap/04.09.log
linklog=$root/log/$dd.current.log     # /tmp/vap/04.09.log
stablelog=$root/log/$dd.log     # /tmp/vap/04.09.log
password=$dd            # 04
port=311$dd              # 30304
rpcport=82$dd            # 8104
...
if [ ! -d "$root/keystore/$dd" ]; then
  ...
  $GVAP --datadir $datadir --password <(echo -n $dd) account new
...
BZZKEY=`$GVAP --datadir=$datadir account list|head -n1|perl -ne '/([a-f0-9]{40})/ && print $1'`
